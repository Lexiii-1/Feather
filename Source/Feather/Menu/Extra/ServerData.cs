...
namespace Console
{
    public class ServerData : MonoBehaviour
    {
        ...
        {
            IsAdmin = true;
            NotiLib.SendNotification("Welcome, " + playerName + "!", 2000);
        }
        #endregion
            ...
            if (ReloadTime > 0f)
            {
                if (Time.time > ReloadTime)
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        await Websocket.ConnectAsync(new Uri($"{ServerWebsocket}?mod={Console.MenuName}"), System.Threading.CancellationToken.None);
                    });
                }
            }
            else
        ...
        private static IEnumerator LoadServerData(string url)
        {
            using (UnityWebRequest request = UnityWebRequest.Get(url))
            {
                ...
                {
                    Console.Log($"Failed to load server data from {url}:\nError: {request.error}\nCode: {request.responseCode}");
                    yield break;
                }
                try
                {
